...
test2View::test2View(QWidget *parent)
    : QWidget(parent),
      DCOPObject("test2Iface")
{
...
void test2View::print(QPainter *p, int height, int width)
{
    // do the actual printing, here
    // p->drawText(etc..)
}

QString test2View::currentURL()
{
    return m_html->url().url();
}

void test2View::openURL(QString url)
{
    openURL(KURL(url));
}

void test2View::openURL(const KURL& url)
{
    m_html->openURL(url);
}

void test2View::slotOnURL(const QString& url)
{
    emit signalChangeStatusbar(url);
}

void test2View::slotSetTitle(const QString& title)
{
    emit signalChangeCaption(title);
}
#include "test2view.moc"

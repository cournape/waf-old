...
try:
    ...
    class AnsiTerm(object):
        ...
        def screen_buffer_info(self):
            ...
            windll.kernel32.GetConsoleScreenBufferInfo(self.hconsole, 
                byref(sbinfo))
            return sbinfo
        ...
        def clear_line(self, param):
            ...
            if mode == 1: # Clear from begining of line to cursor position
                ...
                line_length = sbinfo.Size.X - sbinfo.CursorPosition.X
            chars_written = c_int()
            windll.kernel32.FillConsoleOutputCharacterA(self.hconsole, 
                c_char(' '), line_length, line_start, byref(chars_written))
            windll.kernel32.FillConsoleOutputAttribute(self.hconsole, 
                sbinfo.Attributes, line_length, line_start, byref(chars_written))
        ...
        def clear_screen(self, param):
            ...
            chars_written = c_int()
            windll.kernel32.FillConsoleOutputCharacterA(self.hconsole, 
                c_char(' '), clear_length, clear_start, byref(chars_written))
            windll.kernel32.FillConsoleOutputAttribute(self.hconsole, 
                sbinfo.Attributes, clear_length, clear_start, byref(chars_written))
            ...
            for param, cmd, txt in self.ansi_tokans.findall(text):
                if cmd:
                    ...
                    if isinstance(txt, unicode):
                        windll.kernel32.WriteConsoleW(self.hconsole, txt, 
                            len(txt), byref(chars_written), None)
                    else:
                        windll.kernel32.WriteConsoleA(self.hconsole, txt, 
                            len(txt), byref(chars_written), None)
